...
int GetLD_RAR(int *x)           { return(x[LD_RAR]); }
...
void ieHandler(int vectorCode) {
	...
	if (vectorCode == 1) {
		...
		switch (vectorCode) {
			case 2 :
				printf("Page Fault EXC\n");
				break;
			case 3 :
				printf("Unaligned Access EXC\n");
				break;
			case 4 :
				printf("Protection EXC\n");
				break;
            case 5 :
                printf("Unknown Opcode EXC\n");
                break;
		}
	}
...
void eval_MARMUX(void) {
	// TRAP = 0, ADDRESS = 1
    int marMuxCode = GetMARMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    switch (marMuxCode) {
        case 0 :
        ...
        case 1 :
            ...
            break;
        case 2 :
            gateMARMUXVal = mask(CURRENT_LATCHES.VA, 0x01FF) || mask(CURRENT_LATCHES.PTER, 0x3E00);
            break;
        case 3 :
            gateMARMUXVal = Low16bits((mask(CURRENT_LATCHES.VA, 0xFE00) >> 8) + mask(CURRENT_LATCHES.PTBR, 0xFF00));
            break;
        default :
            printf("Invalid MARMUX select code\n");
            break;
    }
...
void eval_MDR(void) {
    ...
    if (GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        if (vmFlag && CURRENT_LATCHES.STATE_NUMBER == 45) {
            // protection exception check
            if (gateMDRVal >= 0x0000 && gateMDRVal <= 0x2FFF && !supervisorMode)
                ieHandler(0x04);
            // page fault exception check
            else if (mask(gateMDRVal, 0x0004) != 1)
                ieHandler(0x02);
        }
...
void latch_MAR() {
    int marMuxCode = GetMARMUX(CURRENT_LATCHES.MICROINSTRUCTION); 
	if (marMuxCode == 1 || marMuxCode == 3) {
        printf("Supervisor Mode: %d\n", supervisorMode);
        // unaligned access exception check
        if (BUS % 2 && GetLSHF1(CURRENT_LATCHES.MICROINSTRUCTION))
            ieHandler(0x03);
		// // protection exception check
		// if (BUS >= 0x0000 && BUS <= 0x2FFF && !supervisorMode)
		// 	ieHandler(0x02);
  //       // page fault exception check
		// else if (mask(BUS, 0x0004) != 1)
...
void write_to_mem() {
	...
	if (GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION)) {
        if (vmFlag) {
    		MEMORY[CURRENT_LATCHES.MAR/2][1] = Low8bits(mask(CURRENT_LATCHES.PTER, 0xff00) >> 8);
            MEMORY[CURRENT_LATCHES.MAR/2][0] = Low8bits(mask(CURRENT_LATCHES.PTER, 0x00ff));
        }
        else {
...
int gateVAVal;
void eval_VA() {
    gateVAVal = CURRENT_LATCHES.VA;
}

int gatePTBRVal;
void eval_PTBR() {
    gatePTBRVal = CURRENT_LATCHES.PTBR;
}

int gatePTERVal;
void eval_PTER() {
    gatePTERVal = CURRENT_LATCHES.PTER || 0x0001;

    if (CURRENT_LATCHES.RAR == 23 || CURRENT_LATCHES.RAR == 24)
        gatePTERVal = CURRENT_LATCHES.PTER || 0x0002;
}
...
void latch_RAR() {
    NEXT_LATCHES.RAR = NEXT_LATCHES.STATE_NUMBER;
}

void latch_PTER() {
    NEXT_LATCHES.PTER = BUS;
}

int vmStateLUT(int stateCode) {
    switch(stateCode) {
        case 2 :
            return (vmFlag == 0 ? 59 : 29);
        case 3 :
            return (vmFlag == 0 ? 59 : 24);
        case 6 :
            return (vmFlag == 0 ? 59 : 25);
        case 7 :
            return (vmFlag == 0 ? 59 : 23);
        case 18 :
            return (vmFlag == 0 ? 59 : 33);
        case 19 :
            return (vmFlag == 0 ? 59 : 33);
        case 59 :
            return 60;
        case 60 :
            return (CURRENT_LATCHES.READY == 1 ? 45 : 60);
        case 45 :
            return 61;
        case 61 :
            return 62;
        case 62 :
            return (CURRENT_LATCHES.READY == 1 ? 63 : 62);
        case 63 :
            vmFlag = 0;
            return CURRENT_LATCHES.RAR;
        default :
            return NEXT_LATCHES.STATE_NUMBER;
    }
}
...
void eval_micro_sequencer() {
    ...
    */
    ...
    NEXT_LATCHES.STATE_NUMBER = vmStateLUT(CURRENT_LATCHES.STATE_NUMBER);
...
void drive_bus() {
    ...
    */
    ...
    // lab5 functions
    else if (GetGATE_VA(CURRENT_LATCHES.MICROINSTRUCTION))      BUS = Low16bits(gateVAVal);
    else if (GetGATE_PTBR(CURRENT_LATCHES.MICROINSTRUCTION))    BUS = Low16bits(gatePTBRVal);
    else if (GetGATE_PTER(CURRENT_LATCHES.MICROINSTRUCTION))    BUS = Low16bits(gatePTERVal);
...
void latch_datapath_values() {
    ...
    */
    ...
    // lab5 functions
    if (GetLD_VA(CURRENT_LATCHES.MICROINSTRUCTION))     latch_VA();
    //if (GetLD_RAR(CURRENT_LATCHES.MICROINSTRUCTION))    latch_RAR();
    if (GetLD_PTER(CURRENT_LATCHES.MICROINSTRUCTION))   latch_PTER();
}

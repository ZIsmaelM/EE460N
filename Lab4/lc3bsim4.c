...
int stateLUT(int currentState) {
    ...
    switch(currentState) {
        ...
        case 49 :
            nextState = 48;
        case 48 :
            nextState = (SRMS == 1 ? 54 : 50);
        case 54 :
            nextState = (INTMS == 1 ? 58 : 56);
            INTMS != INTMS;
        case 56 :
            nextState = 48;
        case 58 :
            nextState = 50;
        case 50 :
            nextState = (EXCMS == 0 ? 53 : 52);
            EXCMS != EXCMS;
        case 53 :
            nextState = (SRMS == 1 ? 18 : 48);
            SRMS != SRMS;
        case 52 :
            nextState = 60;
        case 60 :
   ...
   */
    if(GetMIO_EN(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        if (CURRENT_LATCHES.READY) {
            ...
            {
                // word
                if (GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION) && CURRENT_LATCHES.MAR % 2 == 0) {
                    MEMORY[CURRENT_LATCHES.MAR/2][1] = hiByteWrite;
                    MEMORY[CURRENT_LATCHES.MAR/2][0] = lowByteWrite;
                }
                else if (GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION) && CURRENT_LATCHES.MAR % 2 == 1) {
                    supervisorMode = 1;
                    NEXT_LATCHES.STATE_NUMBER = stateLUT(CURRENT_LATCHES.STATE_NUMBER);
...
int gatePSRVal, gateSPTRRVal, gateSRRVal, gateNPCRVal;
void eval_bus_drivers() {
   ...
   */
    ...
    if (GetGATE_NPCR(CURRENT_LATCHES.MICROINSTRUCTION)) {
        gateNPCRVal = CURRENT_LATCHES.NPCR;
    }
    if (GetGATE_SRR(CURRENT_LATCHES.MICROINSTRUCTION)) {
        gateSRRVal = CURRENT_LATCHES.VTBR + (CURRENT_LATCHES.SRR << 1);
    }
    if (GetGATE_SPTRR(CURRENT_LATCHES.MICROINSTRUCTION)) {
    ...
    if (GetGATE_ALU(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        if (supervisorMode) {
            gateALUVal = CURRENT_LATCHES.REGS[6];
        }
...
void drive_bus() {
   ...
   */
    ...
    if (GetGATE_PSR(CURRENT_LATCHES.MICROINSTRUCTION))
        BUS = Low16bits(gatePSRVal);
    if (GetGATE_SPTRR(CURRENT_LATCHES.MICROINSTRUCTION))
        BUS = Low16bits(gateSPTRRVal);
    if (GetGATE_SRR(CURRENT_LATCHES.MICROINSTRUCTION))
        BUS = Low16bits(gateSRRVal);
    if (GetGATE_NPCR(CURRENT_LATCHES.MICROINSTRUCTION))
        BUS = Low16bits(gateNPCRVal);
...
void latch_datapath_values() {
   ...
   */
    ...
    if (GetLD_SSPR(CURRENT_LATCHES.MICROINSTRUCTION)) {
    ...
    if (GetLD_SPTRR(CURRENT_LATCHES.MICROINSTRUCTION)) {
        int offset = 0;
        if (CURRENT_LATCHES.STATE_NUMBER == 8)
            offset += 2;
        else if (CURRENT_LATCHES.STATE_NUMBER == 49)
            offset = offset;
        else if (supervisorMode)
            offset -= 2;

        NEXT_LATCHES.SPTRR = BUS + offset;

        // if (CURRENT_LATCHES.STATE_NUMBER == 51) {
        //     NEXT_LATCHES.SPTRR = CURRENT_LATCHES.USPR;
        // }
    }

    if (GetLD_USPR(CURRENT_LATCHES.MICROINSTRUCTION)) {
        NEXT_LATCHES.USPR = BUS;
    }

    if (GetLD_SRR(CURRENT_LATCHES.MICROINSTRUCTION)) {
        if (SRMS)
            NEXT_LATCHES.SRR = CURRENT_LATCHES.INTV;
        else
            NEXT_LATCHES.SRR = CURRENT_LATCHES.EXCV;
    }

    if (GetLD_NPCR(CURRENT_LATCHES.MICROINSTRUCTION)) {
        if (SRMS)
            NEXT_LATCHES.NPCR = CURRENT_LATCHES.PC;
        else
            NEXT_LATCHES.NPCR = CURRENT_LATCHES.PC - 2;
    }

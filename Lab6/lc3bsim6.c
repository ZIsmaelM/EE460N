...
int eval_Address() {
	// PC = 0, BaseReg = 1
	int addr1 = (Get_ADDR1MUX(PS.AGEX_CS) ? PS.AGEX_SR1 : PS.AGEX_NPC);

	// ZERO = 0, offset6 = 1, PCoffset9 = 2, PCoffset11 = 3
	int addr2;
	int addr2MuxCode = Get_ADDR2MUX(PS.AGEX_CS);
	switch (addr2MuxCode) {
		case 0 :
			addr2 = 0;
			break;
		case 1 :
			addr2 = sext(mask(PS.AGEX_IR, 0x003F), 6);
			break;
		case 2 :
			addr2 = sext(mask(PS.AGEX_IR, 0x01FF), 9);
			break;
		case 3 :
			addr2 = sext(mask(PS.AGEX_IR, 0x07FF), 11);
			break;
		default :
...
int eval_Alu() {
  ...
  if (Get_SR2MUX(PS.AGEX_CS)) {
    source2 = Low16bits(mask(PS.AGEX_IR, 0x0001F));
...
int eval_Shf() {
  int shfCode = mask_shfR(PS.AGEX_IR, 0x0030, 4);
  int shfAmount = mask(PS.AGEX_IR, 0x000F);
  int shfResult;
  ...
  switch (shfCode) {
    case 0 :
      shfResult = Low16bits(data << shfAmount);
      break;
    case 1 :
      shfResult = Low16bits(data >> shfAmount);
      break;
    case 2 :
      ...
      break;
    case 3 :
      shfResult = (negFlag ? (Low16bits((data >> shfAmount) | maskBits))
...
int eval_Dependecy(int CS_ADDRESS, int sr1, int sr2) {
  int sr1Flag = Get_SR1_NEEDED(CONTROL_STORE[CS_ADDRESS]);
  int sr2Flag = Get_SR2_NEEDED(CONTROL_STORE[CS_ADDRESS]);
  int memDepend = (sr1Flag && (sr1 == PS.MEM_DRID)) || (sr2Flag && (sr2 == PS.MEM_DRID));
  int srDepend = (sr1Flag && (sr1 == PS.SR_DRID)) || (sr2Flag && (sr2 == PS.SR_DRID));
  int agexDepend = (sr1Flag && (sr1 == PS.AGEX_DRID)) || (sr2Flag && (sr2 == PS.AGEX_DRID));

  if (PS.DE_V) {
    if (v_mem_ld_reg && memDepend) return 1;
    else if (v_sr_ld_reg && srDepend) return 1;
    else if (v_agex_ld_reg && agexDepend) return 1;
    else if (CONTROL_STORE[CS_ADDRESS][BR_OP] && (v_mem_ld_cc || v_sr_ld_cc || v_agex_ld_cc))
      return 1;
    else
      return 0;
  }
...
void MEM_stage() {
  ...
  // D_Cache Data Logic Block
  if (Get_DATA_SIZE(PS.MEM_CS))
    dCacheIn = PS.MEM_ALU_RESULT;
  else if (PS.MEM_ADDRESS % 2)
    dCacheIn = Low16bits((PS.MEM_ALU_RESULT << 8) | mask(PS.MEM_ALU_RESULT, 0x00FF));
  else
    dCacheIn = PS.MEM_ALU_RESULT;
  ...
  int dcache_r;
  if (v_dcache_en)
    dcache_access(PS.MEM_ADDRESS, &dCacheOut, dCacheIn, &dcache_r, WE0, WE1);
  else
    dcache_r = 1;
  ...
  if (v_dcache_en) {
    if (Get_DATA_SIZE(PS.MEM_CS))
      TRAP_PC = Low16bits(dCacheOut);
    else if (PS.MEM_ADDRESS % 2)
      TRAP_PC = Low16bits(mask_shfR(dCacheOut, 0xFF00, 8));
    else
      TRAP_PC = Low16bits(mask(dCacheOut, 0x00FF));
  ...
  if (PS.MEM_V) {
    if (Get_BR_OP(PS.MEM_CS)) {
      int n = mask_shfR(PS.MEM_CC, 0x0004, 2) & mask_shfR(PS.MEM_IR, 0x0800, 11);
      int z = mask_shfR(PS.MEM_CC, 0x0002, 1) & mask_shfR(PS.MEM_IR, 0x0400, 10);
      int p = mask(PS.MEM_CC, 0x0001) & mask_shfR(PS.MEM_IR, 0x0200, 9);
      int ben = n | z | p;
      MEM_PCMUX = (ben ? 1 : MEM_PCMUX);
    }
...
void AGEX_stage() {
  ...
  if (Get_ADDRESSMUX(PS.AGEX_CS))
  	...
  	addr = Low16bits(mask(PS.AGEX_IR, 0x00FF) << 1);
...
void DE_stage() {
  ...
  int sr2;
  if(mask_shfR(PS.DE_IR, 0x2000, 13))
  	sr2 = mask_shfR(PS.DE_IR, 0x0E00, 9);
  else
  ...
  dep_stall = eval_Dependecy(CONTROL_STORE_ADDRESS, sr1, sr2);
...
void FETCH_stage() {
  ...
  int decodeStall = !dep_stall && !v_de_br_stall;
  int agexStall = !v_agex_br_stall;
  int memStall = !mem_stall && !v_mem_br_stall;
  int stallFlag = decodeStall && agexStall && memStall;

  int LD_PC;
  if (icache_r && stallFlag)
    LD_PC = 1;
  ...
  // stall check
  int decodeV = ((!LD_PC || v_mem_br_stall) ? 0 : 1);
